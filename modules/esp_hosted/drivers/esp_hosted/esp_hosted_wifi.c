...
#define DT_DRV_COMPAT espressif_esp_hosted_sdio


#define SDHC_NODE DT_BUS(DT_DRV_INST(0))
#define WIFI_NODE DT_DRV_INST(0)
#define WIFI_FUNC_NUM DT_REG_ADDR(WIFI_NODE)
...
static esp_hosted_config_t esp_hosted_config = {
	...
	//.sdio_int_gpio = GPIO_DT_SPEC_INST_GET_OR(0, sdio_int_gpios, {0}),
	.sdhc_dev = DEVICE_DT_GET(SDHC_NODE)   //DEVICE_DT_INST_GET(0)
};
			      ...
			      uint32_t timeout)
{
	...
	if (esp_hosted_hal_sdio_transfer(dev, &frame, NULL, ESP_FRAME_SIZE_ROUND(frame))) {
	...
	for (;; k_msleep(CONFIG_WIFI_ESP_HOSTED_EVENT_TASK_POLL_MS)) {
		...
		do {
			...
			if (((ESP_FRAME_SIZE * 2) - frame.len) < ESP_FRAME_SIZE) {
				...
				LOG_ERR("SDIO buffer overflow offset: %d", frame.len);
				return;
			}

			if (esp_hosted_hal_sdio_transfer(dev, NULL, &ffrag, ESP_FRAME_SIZE)) {
				goto restart;
			}
...
static int esp_hosted_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	if (pkt_len > ESP_FRAME_MAX_PAYLOAD) {
		LOG_ERR("packet length > SDIO buf length");
		return -ENOMEM;
	}
